...
<body class="center-page subject-page"><a class="skip-link" href="#main">본문 바로가기</a>{navigation("../../", "과목별학원")}
  <main id="main">
    ...
    <section class="local-section"><div class="wrap local-grid"><article class="local-card"><p class="eyebrow">ANSWER READY</p><h2>{esc(copy['label'])}, 무엇부터 확인할까요?</h2><p>{esc(copy['summary'])}</p></article><article class="local-card"><p class="eyebrow">CONSULTATION CHECK</p><h2>상담 전 준비할 기록</h2><ul class="summary-list"><li>최근 시험지와 현재 교재</li><li>틀린 문제의 답안 또는 풀이 흔적</li><li>학교 시험 범위와 수행 일정</li><li>일주일 공부 시간과 과제 기록</li></ul></article></div></section>
...
<body class="local-page child-page subject-page"><a class="skip-link" href="#main">본문 바로가기</a>{navigation("../../../", "과목별학원")}
  <main id="main">
    ...
    <section id="verified-center" class="local-section"><div class="wrap verified-center-grid"><article class="verified-center-card"><p class="eyebrow">VERIFIED CENTER DATA</p><h2>확인된 센터 정보</h2><dl class="verified-data-list">{data_rows}</dl>{f'<div class="verified-school-list">{school_html}</div>' if school_html else '<p class="verified-note">제공 자료에 학교 목록이 없어 특정 학교 진도를 임의로 단정하지 않습니다.</p>'}{tuition}<p class="verified-note">자료 기준: 센터정보 정리 자료 · 최종 검수 {TODAY}</p></article><article class="local-card subject-answer-card"><p class="eyebrow">ANSWER READY</p><h2>{esc(manuscript.get('answer_heading') or f'{local} 상담 판단 기준')}</h2><p>{esc(manuscript.get('answer_text') or summary)}</p><div class="pill-list">{"".join(f'<span>{esc(tag)}</span>' for tag in manuscript.get('answer_tags', []))}</div></article></div></section>
