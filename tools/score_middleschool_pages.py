#!/usr/bin/env python3
"""Strict 100-point audit for 코칭학원.com 중학생학원 detail pages.
...
WEIGHTS: dict[str, int] = {
    ...
    "factual_middle_grades_visible": 6,
    "factual_middle_grades_schema": 4,
    "factual_middle_schools_visible": 5,
    "factual_middle_schools_schema": 4,
...
GRADE_RE = re.compile(r"(?<![가-힣A-Za-z0-9])(?P<grade>[초중고][1-6])(?![0-9])")
MIDDLE_GRADE_RE = re.compile(r"중[1-3]")
SCHOOL_NAME_RE = re.compile(r"[가-힣A-Za-z0-9]+?(?:초등학교|중학교|고등학교|초|중|고)$")

READER_RE = re.compile(r"중학생|학생|자녀|아이|학부모|보호자")
...
AUTHORING_RE = re.compile(
    r"(?<![가-힣])원고(?![가-힣])|프롬프트|작성\s*지시|다음과\s*같이\s*작성|메타\s*설명|"
    r"검색\s*키워드|(?<![A-Za-z])SEO(?![A-Za-z])|JSON-LD|구조화\s*데이터|"
    r"D열|분량을\s*맞|본문에\s*삽입",
    re.IGNORECASE,
)
UNVERIFIED_OPERATION_RE = re.compile(
    r"입시(?:컨설팅|합격관리|합격전략|성공사례|자료분석|일정관리|로드맵|"
    r"준비반|캠프|실적|특강|설계|분석|평가|결과)|"
    r"방학(?:특강|캠프)|셔틀|주말(?:집중반|수업)|오전수업|"
    r"온라인수업|화상수업|녹화수업|실시간수업|대면수업|"
    r"소그룹수업|그룹수업|소수정예수업|일대일수업|정원제수업|"
    r"집중수업|밀착관리수업|수준별수업|보충수업|학습클리닉반|"
    r"자습실|스터디룸|강의실|휴게실|사물함"
)
POSITIVE_RESULT_RE = re.compile(
    r"(?:성적|점수|등급|합격|입시\s*결과).{0,28}"
    r"(?:향상|상승|개선|달성|보장|성공|사례|증명)|"
...
def is_middle_school(value: str) -> bool:
    return bool(re.search(r"(?:중학교|중)$", value))
...
def grade_tokens(value: Any) -> list[str]:
    if isinstance(value, list):
        ...
        source = str(value or "")
    return unique(match.group("grade") for match in GRADE_RE.finditer(source))
...
def expected_middle_grades(row: dict[str, str]) -> list[str]:
    english_grades = split_values(row.get("가능학년\n(영어)", ""))
    math_grades = set(split_values(row.get("가능학년\n(수학)", "")))
    return [grade for grade in english_grades if grade in math_grades and MIDDLE_GRADE_RE.fullmatch(grade)]


def expected_middle_schools(row: dict[str, str]) -> list[str]:
    scope_values = {"지역내 모든 중학교 가능", "지역 내 모든 중학교 가능"}
    raw = [
        value
        for value in split_school_values(row.get("타깃학교\n(중)", ""))
        if value not in scope_values and value != "오현초호매실중"
    ]
...
def load_source_model(path: Path) -> SourceModel:
    ...
    for row in rows:
        ...
        if not locality:
            ...
            continue
        middle_grades = expected_middle_grades(row)
        middle_schools = expected_middle_schools(row)
        invalid_middle = [name for name in middle_schools if not is_middle_school(name)]
        if invalid_middle:
            errors.append(f"center-middle-school-invalid:{display}:{invalid_middle}")
        ...
        mapped[locality] = SourceRow(
            ...
            values=row,
            middle_grades=tuple(middle_grades),
            middle_schools=tuple(middle_schools),
...
def schema_grade_checks(nodes: list[dict[str, Any]], expected: list[str]) -> tuple[bool, str]:
    ...
    if any(not MIDDLE_GRADE_RE.fullmatch(value) for value in schema_grades):
...
def known_school_regex(names: Iterable[str]) -> re.Pattern[str] | None:
    escaped = [re.escape(name) for name in sorted(set(names), key=len, reverse=True) if name]
    if not escaped:
        return None
    return re.compile(
...
def initial_record(
    ...
    if not schema_ok:
        details["technical_schema"] = f"json_errors={json_errors} missing_types={sorted(REQUIRED_SCHEMA_TYPES - types)} forbidden={sorted(FORBIDDEN_SCHEMA_TYPES & types)} webpage={len(webpages)} article={len(articles)}"
    ...
    expected_grades = list(source_row.middle_grades)
    ...
    criteria["factual_middle_grades_visible"] = visible_grade_ok
    if not visible_grade_ok:
        details["factual_middle_grades_visible"] = f"actual={visible_grades} expected={expected_grades} raw={visible_grade_text!r}"

    grade_schema_ok, grade_schema_detail = schema_grade_checks(nodes, expected_grades)
    criteria["factual_middle_grades_schema"] = grade_schema_ok
    if not grade_schema_ok:
        details["factual_middle_grades_schema"] = grade_schema_detail

    expected_schools = list(source_row.middle_schools)
    ...
    criteria["factual_middle_schools_visible"] = visible_school_ok
    if not visible_school_ok:
        details["factual_middle_schools_visible"] = f"actual={visible_schools} expected={expected_schools}"
    ...
    criteria["factual_middle_schools_schema"] = school_schema_ok
    if not school_schema_ok:
        details["factual_middle_schools_schema"] = f"webpage={webpage_schools} article={article_schools} expected={expected_schools}"
    ...
    relevant_grades = grade_tokens(article_text + " " + " ".join(q + " " + a for q, a in screen_faq) + " " + visible_grade_text + " " + schema_text)
    cross_level = [grade for grade in relevant_grades if not MIDDLE_GRADE_RE.fullmatch(grade)]
    all_declared_middle = all(is_middle_school(name) for name in visible_schools + webpage_schools + article_schools)
    criteria["factual_no_cross_level"] = not unexpected_schools and not cross_level and all_declared_middle
    if not criteria["factual_no_cross_level"]:
        details["factual_no_cross_level"] = f"unexpected_schools={unexpected_schools[:10]} cross_level_grades={cross_level} declared_middle_only={all_declared_middle}"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "코칭학원.com 중학생학원 371개 페이지를 100점 기준으로 채점합니다. "
            "점수는 검색순위 예측이 아닙니다."
        )
    )
    parser.add_argument(
