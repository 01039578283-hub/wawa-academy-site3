#!/usr/bin/env python3
"""Strict 100-point audit for 코칭학원.com 고등학생학원 detail pages.
...
CATEGORY_SLUG = "고등학생학원"
CATEGORY_LABEL = "고등학생학원"
...
WEIGHTS: dict[str, int] = {
    ...
    "factual_high_grades_visible": 6,
    "factual_high_grades_schema": 4,
    "factual_high_schools_visible": 5,
    "factual_high_schools_schema": 4,
...
HIGH_GRADE_RE = re.compile(r"고[1-3]")
SCHOOL_NAME_RE = re.compile(r"[가-힣A-Za-z0-9]+?(?:초등학교|중학교|고등학교|초|중|고)$")

READER_RE = re.compile(r"고등학생|학생|자녀|아이|학부모|보호자")
...
def is_high_school(value: str) -> bool:
    return bool(re.search(r"(?:고등학교|고)$", value))
...
def expected_high_grades(row: dict[str, str]) -> list[str]:
    english_grades = split_values(row.get("가능학년\n(영어)", ""))
    math_grades = set(split_values(row.get("가능학년\n(수학)", "")))
    return [grade for grade in english_grades if grade in math_grades and HIGH_GRADE_RE.fullmatch(grade)]


def expected_high_schools(row: dict[str, str]) -> list[str]:
    scope_values = {"지역내 모든 고등학교 가능", "지역 내 모든 고등학교 가능"}
    raw = [
        value
...
def row_all_schools(row: dict[str, str]) -> list[str]:
    ...
    return public_school_names(
        ...
        if value not in {"지역내 모든 고등학교 가능", "지역 내 모든 고등학교 가능"}
    )
...
def load_source_model(path: Path) -> SourceModel:
    ...
    for row in rows:
        ...
        if not locality:
            ...
            continue
        high_grades = expected_high_grades(row)
        high_schools = expected_high_schools(row)
        invalid_high = [name for name in high_schools if not is_high_school(name)]
        if invalid_high:
            errors.append(f"center-high-school-invalid:{display}:{invalid_high}")
        ...
        mapped[locality] = SourceRow(
            ...
            values=row,
            high_grades=tuple(high_grades),
            high_schools=tuple(high_schools),
...
def schema_grade_checks(nodes: list[dict[str, Any]], expected: list[str]) -> tuple[bool, str]:
    ...
    if any(not HIGH_GRADE_RE.fullmatch(value) for value in schema_grades):
...
def initial_record(
    ...
    expected_grades = list(source_row.high_grades)
    ...
    criteria["factual_high_grades_visible"] = visible_grade_ok
    if not visible_grade_ok:
        details["factual_high_grades_visible"] = f"actual={visible_grades} expected={expected_grades} raw={visible_grade_text!r}"

    grade_schema_ok, grade_schema_detail = schema_grade_checks(nodes, expected_grades)
    criteria["factual_high_grades_schema"] = grade_schema_ok
    if not grade_schema_ok:
        details["factual_high_grades_schema"] = grade_schema_detail

    expected_schools = list(source_row.high_schools)
    ...
    criteria["factual_high_schools_visible"] = visible_school_ok
    if not visible_school_ok:
        details["factual_high_schools_visible"] = f"actual={visible_schools} expected={expected_schools}"
    ...
    criteria["factual_high_schools_schema"] = school_schema_ok
    if not school_schema_ok:
        details["factual_high_schools_schema"] = f"webpage={webpage_schools} article={article_schools} expected={expected_schools}"
    ...
    cross_level = [grade for grade in relevant_grades if not HIGH_GRADE_RE.fullmatch(grade)]
    all_declared_high = all(is_high_school(name) for name in visible_schools + webpage_schools + article_schools)
    criteria["factual_no_cross_level"] = not unexpected_schools and not cross_level and all_declared_high
    if not criteria["factual_no_cross_level"]:
        details["factual_no_cross_level"] = f"unexpected_schools={unexpected_schools[:10]} cross_level_grades={cross_level} declared_high_only={all_declared_high}"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "코칭학원.com 고등학생학원 371개 페이지를 100점 기준으로 채점합니다. "
            "점수는 검색순위 예측이 아닙니다."
        )
    )
    parser.add_argument(
